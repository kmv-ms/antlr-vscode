...
class ErrorParser {
    contexts;
    static errorPattern = /(\w+)\s*\((\d+)\):\s*((?:\w:)?[^:]*):(\d*):(\d*):\s*(.+)/;
    static errorCodeToPattern = new Map([
        [8, /grammar name (\w+)/],
        [56, /:\s+(\w+)/],
                    ...
                    if (firstLevelMatches[4].length > 0) {
                        ...
                        switch (errorCode) {
                            ...
                            case 182: {
                                ...
                                if (matches) {
                                    if (matches.length > 2) {
                                        ...
                                        this.addDiagnosticsForSymbols(symbols, errorText, types_1.DiagnosticType.Error, context);
                                        continue;
                                    }
                            ...
                            case 124: {
                                const matches = ErrorParser.errorCodeToPattern.get(errorCode).exec(errorText);
                                if (matches) {
                                    this.addDiagnosticsForSymbols([matches[1]], errorText, types_1.DiagnosticType.Warning, context);
                                    range.end.column += matches[1].length - 1;
                                }
                                break;
                            }
                        ...
                        const error = {
                            type: (firstLevelMatches[1] === "error") ? types_1.DiagnosticType.Error : types_1.DiagnosticType.Warning,
                            message: errorText,
                            range,
                        };
                        ...
                        switch (errorCode) {
                            ...
                            case 54:
                            case 99:
                                this.addGenericDiagnosis(errorText, types_1.DiagnosticType.Error, context);
                                break;
                            case 119: {
                            ...
                            default:
                                this.addGenericDiagnosis(`[Internal Error] Unhandled error message (code ${errorCode}, message: ` +
                                    ...
                                    "https://github.com/mike-lischke/vscode-antlr4/issues)", types_1.DiagnosticType.Error, context);
                                break;
                        }
                    }
                }
